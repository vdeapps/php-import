...
class ImportAbstract implements ImportInterface
{
    ...
    public function addFields($mixed)
    {
        if (!is_numeric($mixed) && !is_array($mixed)) {
            throw new \Exception(__FUNCTION__ . " doit être un endtier ou un tableau", self::IMPORT_BAD_FORMAT);
     ...
     * @throws \Exception
     */
    public function fromFile($localFilename)
    {
        if (!file_exists($localFilename)) {
            throw new \Exception("Fichier d'import non trouvé: $localFilename", self::IMPORT_FILENOTFOUND);
        }
        
        $resource = fopen($localFilename, "r");
        if ($resource === false) {
            throw new \Exception("Erreur de lecture de $localFilename", self::IMPORT_READ_ERROR);
        }
